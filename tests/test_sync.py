...
class TestSyncToDrive:
    def test_disabled_when_unset(self, tmp_path, monkeypatch, artifacts):
        monkeypatch.setattr(sp, "DRIVE_SYNC_DIR", None)
        sp.sync_to_drive(tmp_path / "recap.md")     # must not raise

    def test_masters_land_flat_and_characters_in_a_subfolder(self, tmp_path, monkeypatch, artifacts):
        drive = tmp_path / "drive"
        monkeypatch.setattr(sp, "DRIVE_SYNC_DIR", str(drive))
        sp.LORE_FILE.write_text("lore\n")
        sp.NPCS_FILE.write_text("npcs\n")
        sp.ALLIES_FILE.write_text("allies\n")
        (artifacts / "characters" / "caeli.md").write_text("# Caeli\n")
        recap = sp.RECAPS_DIR / "08_31_2026_recap.md"
        recap.write_text("# Recap\n")

        sp.sync_to_drive(recap)
    ...
    def test_a_missing_drive_folder_warns_but_does_not_raise(self, tmp_path, monkeypatch, artifacts, capsys):
        ...
        monkeypatch.setattr(sp, "DRIVE_SYNC_DIR", str(blocker / "Inkwell"))
        recap = sp.RECAPS_DIR / "r.md"
        recap.write_text("# Recap\n")
        sp.sync_to_drive(recap)
