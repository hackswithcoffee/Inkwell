...
class TestAppendLoreAndNpcs:
    def test_appends_under_a_dated_heading(self, artifacts):
        sp.append_lore_and_npcs({"lore": "A vault.", "npcs": "A harbourmaster."}, "08_31_2026")
        assert "## Update 08_31_2026\nA vault." in sp.LORE_FILE.read_text()
        assert "## Update 08_31_2026\nA harbourmaster." in sp.NPCS_FILE.read_text()

    def test_second_session_appends_rather_than_replaces(self, artifacts):
        sp.append_lore_and_npcs({"lore": "First."}, "08_10_2026")
        sp.append_lore_and_npcs({"lore": "Second."}, "08_31_2026")
        text = sp.LORE_FILE.read_text()
        assert "First." in text and text.index("First.") < text.index("Second.")

    def test_empty_fields_write_nothing(self, artifacts):
        sp.append_lore_and_npcs({"lore": "", "npcs": ""}, "08_31_2026")
        assert not sp.LORE_FILE.exists() and not sp.NPCS_FILE.exists()


class TestUpdateAlliesRoster:
    def test_new_ally_gets_a_section(self, artifacts):
        sp.update_allies_roster([{"name": "Maren", "status": "Friendly", "notes": "Ferried us."}], "August 10, 2026")
        text = sp.ALLIES_FILE.read_text()
    ...
    def test_returning_ally_is_spliced_into_their_own_section(self, artifacts):
        ...
        sp.update_allies_roster([{"name": "Maren", "notes": "First."}], "August 10, 2026")
        sp.update_allies_roster([{"name": "Odo", "notes": "Met."}], "August 10, 2026")
        sp.update_allies_roster([{"name": "Maren", "notes": "Returned."}], "August 24, 2026")
        text = sp.ALLIES_FILE.read_text()
    ...
    def test_matching_is_by_heading_not_substring(self, artifacts):
        ...
        sp.update_allies_roster([{"name": "Alchemist", "notes": "Sold potions."}], "August 10, 2026")
        sp.update_allies_roster([{"name": "Al", "notes": "A different person."}], "August 24, 2026")
        text = sp.ALLIES_FILE.read_text()
        assert "## Alchemist" in text and "## Al\n" in text

    def test_missing_status_is_recorded_as_unknown(self, artifacts):
        sp.update_allies_roster([{"name": "Maren"}], "August 10, 2026")
        assert "**Status:** Unknown" in sp.ALLIES_FILE.read_text()

    def test_entries_without_a_name_are_skipped(self, artifacts):
        sp.update_allies_roster([{"notes": "nameless"}, "a string", None], "August 10, 2026")
        assert not sp.ALLIES_FILE.exists() or "nameless" not in sp.ALLIES_FILE.read_text()

    def test_no_allies_writes_nothing(self, artifacts):
        sp.update_allies_roster([], "August 10, 2026")
        assert not sp.ALLIES_FILE.exists()


class TestUpdateCharacterFiles:
    def test_creates_a_chronicle_with_a_title(self, artifacts):
        sp.update_character_files([{"name": "Caeli", "development": "Grew."}], "08_31_2026")
